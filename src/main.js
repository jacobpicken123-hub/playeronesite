...
async function cloudPullAllSaves(preferLocalIfNewer = false) {
  ...
  if (preferLocalIfNewer) {
    for (const localId in (state.saves || {})) {
      ...
      if (!cloud) { newSaves[localId] = local; needsRepush = true; continue; }
      ...
      if (cloudNewer && cloudCount >= localCount) {
        // Cloud is the better copy — keep it (already in newSaves).
      } else {
        newSaves[localId] = local;            // keep the complete local copy
        if (cloudCount < localCount) needsRepush = true; // cloud is missing data — heal it
      }
    }
  }
