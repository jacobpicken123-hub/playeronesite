...
function calcDriverCareer(nameKey) {
  ...
  Object.values(state.saves || {}).forEach(save => {
    Object.values(save.seasons || {}).forEach(season => {
      ...
      const championId = standings.find(d => !d.championshipDsq)?.driverId;
      const isChamp = finished && championId === row.driverId;
...
function buildSeasonFromImport(parsed, opts) {
  ...
  // Drop trailing empty rounds — a full-calendar CSV often has more flag columns
  // than rounds actually contested, leaving empty "future" races. Those phantom
  // rounds keep the title mathematically open forever, so a clear champion never
  // gets crowned. Trimming the unraced tail lets the season conclude at its last
  // contested round. (Only the trailing block is removed; mid-season gaps stay.)
  while (season.races.length && !season.races[season.races.length - 1].completed) {
    season.races.pop();
  }
